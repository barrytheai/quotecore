    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-950 dark:text-zinc-50">
      <header className="sticky top-0 z-50 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          ...
          <a href="/" className="inline-flex h-11 items-center justify-center rounded-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-zinc-900 dark:text-zinc-100 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800">
            Back to homepage
          </a>
        </div>
      </header>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((post) => (
              <a
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="block rounded-[1.75rem] border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-7 shadow-sm transition-shadow hover:shadow-md"
              >
                <p className="text-xs text-zinc-400">{post.date}</p>
                <h2 className="mt-2 text-xl font-semibold text-zinc-950 dark:text-zinc-50">{post.title}</h2>
                <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{post.description}</p>
              </a>
            ))
          )}
        </div>
